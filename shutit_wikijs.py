...
class shutit_wikijs(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.build['module_name'] = 'shutit_wikijs_' + ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
  ...
  config.vm.define "wikijs1" do |wikijs1|
    wikijs1.vm.box = ''' + '"' + vagrant_image + '"' + '''
    wikijs1.vm.hostname = "wikijs1.vagrant.test"
    config.vm.provider :virtualbox do |vb|
      vb.name = "shutit_wikijs_1"
    end
  end
end''')
		...
		machines = {}
		machines.update({'wikijs1':{'fqdn':'wikijs1.vagrant.test'}})
...
def module():
	return shutit_wikijs(
		'imiell.shutit_wikijs.shutit_wikijs', 1462463697.0001,
		description='',
		maintainer='',
